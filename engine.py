...
class Engines(Enum):
    ...
    HUGGINGFACE_TIBETAN_ASR = "HUGGINGFACE_TIBETAN_ASR"
...
class Engine(object):
    ...
    @classmethod
    def create(cls, x: Engines, language: Languages, device: str, cache: bool = False, **kwargs):
        if x is Engines.AMAZON_TRANSCRIBE:
            ...
            return WhisperTiny(language=language, device=device)
        elif x is Engines.WHISPER_BASE:
            return WhisperBase(language=language, device=device)
        elif x is Engines.WHISPER_SMALL:
            return WhisperSmall(language=language, device=device, cache=cache)
        elif x is Engines.WHISPER_MEDIUM:
            return WhisperMedium(language=language, device=device, cache=cache)
        elif x is Engines.WHISPER_LARGE:
            return WhisperLarge(language=language, device=device, cache=cache)
        elif x is Engines.WHISPER_LARGE_V2:
            return WhisperLargeV2(language=language, device=device, cache=cache)
        elif x is Engines.WHISPER_LARGE_V3:
            return WhisperLargeV3(language=language, device=device, cache=cache)
        ...
        elif x is Engines.HUGGINGFACE_TIBETAN_ASR:
            return HuggingFaceTibetanASREngine(device=device)
        else:
...
class Whisper(Engine):
    ...
    def __init__(self, cache_extension: str, model: str, language: Languages, device: str, cache: bool = False):
        if not cache:
    ...
    def transcribe(self, path: str) -> str:
        ...
        self._audio_sec += len(audio) / sample_rate
...
class WhisperTiny(Whisper):
    def __init__(self, language: Languages, device: str):
        model = "tiny.en" if language == Languages.EN else "tiny"
        super().__init__(cache_extension=".wspt", model=model, language=language, device=device)
...
class WhisperBase(Whisper):
    def __init__(self, language: Languages, device: str, cache: bool = False):
        model = "base.en" if language == Languages.EN else "base"
        super().__init__(cache_extension=".wspb", model=model, language=language, device=device, cache=cache)
...
class WhisperSmall(Whisper):
    def __init__(self, language: Languages, device: str, cache: bool = False):
        ...
        super().__init__(cache_extension=".wsps", model=model, language=language, device=device, cache=cache)
...
class WhisperMedium(Whisper):
    def __init__(self, language: Languages, device: str, cache: bool = False):
        ...
        super().__init__(cache_extension=".wspm", model=model, language=language, device=device, cache=cache)
...
class WhisperLarge(Whisper):
    def __init__(self, language: Languages, device: str, cache: bool = False):
        super().__init__(cache_extension=".wspl", model="large-v1", language=language, device=device, cache=cache)
...
class WhisperLargeV2(Whisper):
    def __init__(self, language: Languages, device: str, cache: bool = False):
        super().__init__(cache_extension=".wspl2", model="large-v2", language=language, device=device, cache=cache)
...
class WhisperLargeV3(Whisper):
    def __init__(self, language: Languages, device: str, cache: bool = False):
        super().__init__(cache_extension=".wspl3", model="large-v3", language=language, device=device, cache=cache) 
...
class HuggingFaceTibetanASREngine(Engine):
    def __init__(self, device: str = "cpu"):
        model_id = "./whisper-small-finetune"
    ...
    def transcribe(self, path: str) -> str:
        cache_path = path.replace(".flac", ".tib")
        if os.path.exists(cache_path):
    ...
    def __str__(self) -> str:
        return "HuggingFace Tibetan ASR (unwang/tibetan_ASR_small)"
